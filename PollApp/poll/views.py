...
class PollCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        success_url = reverse_lazy(
            'poll:detail', kwargs={'pk': self.object.id}
        )
        return success_url
        ...
        for i in range(5):
            ...
            if choice in self.request.POST:
                Choice.objects.create(
                    poll=self.object, choice=self.request.POST[choice]
                )
        return response
...
class PollEditView(OwnerEditMixin, UpdateView):
    ...
    def get_success_url(self):
        success_url = reverse_lazy('poll:detail', kwargs={'pk': self.pk})
        return success_url
...
class PollDetailView(LoginRequiredMixin, TemplateResponseMixin, View):
    ...
    def get_selected_choice(
        self, user: CustomUser, queryset: QuerySet
    ) -> Optional[Choice]:
    ...
    def get_context_data(self, **kwargs):
        context = kwargs
        selected_choice = self.request.user.get_selected_choice(
            self.poll.choices.all()
        )
    ...
    def post(self, request, pk):
        ...
        # update fields
        self.poll.update_vote(
            selected_choice=selected_choice, user=request.user
        )
